...
describe('notification policy',()=>{
 ...
 it('formats a concise deal alert without diagnostics',()=>{const text=formatTelegramDealAlert({...listing,district:'Ķengarags',address:'Dubnas 8',areaM2:36,url:'https://example.test'} as Listing,{...result,renovationCostEur:15372,baseArvEur:73924,expectedProfitEur:4754,roi:.076} as UnderwritingResult);expect(text).toContain('🏠 Ķengarags, Dubnas 8');expect(text).toContain('📈 Profit: €4,754');expect(text).not.toMatch(/risk|confidence|comps|status/i);});
});
