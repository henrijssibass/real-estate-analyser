...
import { hasImportantSearchChange } from '../src/history.js';
...
describe('notification policy',()=>{
 ...
 it('formats a concise deal alert with status, floor and cost ranges',()=>{const text=formatTelegramDealAlert({...listing,district:'Ķengarags',address:'Dubnas 8',areaM2:36,floor:4,totalFloors:9,url:'https://example.test'} as Listing,{...result,renovationCostBeforeContingencyEur:14640,renovationCostEur:15372,baseArvEur:73924,underwritingArvEur:73924,expectedProfitEur:4754,expectedProfitLowEur:4754,expectedProfitHighEur:5486,roi:.076} as UnderwritingResult);expect(text).toContain('🏠 PASS — Ķengarags, Dubnas 8');expect(text).toContain('🏢 Floor: 4/9');expect(text).toContain('🔨 Renovation: €14,640–€15,372');expect(text).toContain('🏦 ARV: €73,924');expect(text).toContain('📈 Profit: €4,754–€5,486');expect(text).not.toMatch(/risk|confidence|comps/i);});
});
