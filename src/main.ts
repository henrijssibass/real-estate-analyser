  ...
  async requestHandler({request,$,enqueueLinks}) {
    ...
    if (request.userData.label==='SEARCH') {
      ...
      for (const listing of listings) { if (discovered>=maxListings) break;
        ...
        if(previous&&!hasImportantSearchChange(previous,listing)){skippedOldListings++;await markSeenUnchanged(store,previous,listing,input.dryRun??false);continue;}
        pageHasNewOrChanged=true; if(previous) changedListings++; else newListings++;
      ...
      if (discovered<maxListings&&pageHasNewOrChanged) for (const url of paginationUrls($,request.url)) await queue.addRequest({url,userData:request.userData});
...
async function processListing(listing:Listing,seriesFocus?:string,roomsFocus?:string) {
  if(!matchesFocus(listing,seriesFocus,roomsFocus)){filteredOut++;return;}
