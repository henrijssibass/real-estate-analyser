...
function loadConfig_() {
  ...
  return {comps:comps,reno:reno,costs:costs,filters:filters};
}
...
function underwrite_(l,cfg,minProfit) {
  ...
  const other=base==null?null:Number(l.priceEur||0)*purchasePct+base*salePct+fixed; const total=reno==null||other==null?null:Number(l.priceEur||0)+reno+other;
  const profit=base==null||total==null?null:base-total; const roi=profit==null||!total?null:profit/total; const margin=profit==null||!base?null:profit/base;
...
function upsertRaw_(l) {
  const sh=SpreadsheetApp.getActive().getSheetByName(RAW_SHEET); const last=Math.max(4,sh.getLastRow());
  ...
  const values=[[l.listingId,safeDate_(l.scrapedAt),l.source,l.searchName,l.district,l.address,l.series,l.rooms,l.bathrooms,l.areaM2,l.floor,l.totalFloors,l.priceEur,l.pricePerM2,l.title,l.description,l.url,(l.imageUrls||[]).join('\n'),'','','','',safeDate_(l.publishedAt),safeDate_(l.firstSeenAt||l.scrapedAt),Boolean(l.isNew)]];
  sh.getRange(row,1,1,25).setValues(values);
}
